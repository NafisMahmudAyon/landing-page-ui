...
export const Avatar = ({ styles = "", name, src, altText, imageStyles = "", variant, onClick, ...rest }) => {
    ...
    return (_jsxs("div", { className: `${styles} ${variantValue.styles} flex justify-center items-center w-12 h-12 rounded-full overflow-hidden`, ...(name && { title: name }), ...rest, onClick: onClick, children: [src && (_jsx("img", { src: src, alt: altText || "alt text", className: `${imageStyles} ${variantValue.imageStyles} max-w-full h-auto ` })), !src && name && _jsx(_Fragment, { children: nameX || "A" }), !src && !name && _jsx(Icon, { icon: "fa-user", iconLibrary: "font-awesome" })] }));
};
